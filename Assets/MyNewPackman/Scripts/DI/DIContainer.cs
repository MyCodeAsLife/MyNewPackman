...
public class DIContainer
{
    ...
    public void RegisterInstance<T>(string tag, T instance)     // Регистрация конкретного экземпляра
    {
        var key = (tag, typeof(T));
    ...
    public T Resolve<T>(string tag = null)
    {
        ...
        throw new Exception($"Couldn't find dependency for tag {tag} an type {key.Item2.FullName}");
    }

    private void Register<T>((string, Type) key, Func<DIContainer, T> factory, bool isSingleton)    // Регистрация фабрики
    {
        if (_registrations.ContainsKey(key))
